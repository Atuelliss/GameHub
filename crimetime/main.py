...
########## Information Commands ##########
    ...
    # Check if the bot has permission to send embeds
        ...
        embed = discord.Embed(
            title="CrimeTime",
            description=(
                "A criminal mini-game for Red-DiscordBot by Jayar(Vainne).\n"
                "Accumulate wealth through various criminal activities!"
            ),
            color=0xffd700
        )
        ...
        embed.add_field(
            name="Crime Commands",
            value=(
                f"`{p}mug` - Mug NPCs or players\n"
                f"`{p}mug consider <user>` - Compare odds vs a player\n"
                f"`{p}cttarget` - View recent mug targets"
            ),
            inline=False
        )
        ...
        embed.add_field(
            name="Stats and Info",
            value=(
                f"`{p}ctstat` - View your wealth, gear, and stats\n"
                f"`{p}ctwealth` - View total wealth breakdown\n"
                f"`{p}mugcheck` - View mug ratio and balance\n"
                f"`{p}mugclear` - Reset your mug stats (costs cash)"
            ),
            inline=False
        )
        ...
        embed.add_field(
            name="Economy",
            value=(
                f"`{p}ctinvest` - Convert cash to gold bars or gems\n"
                f"`{p}ctliquidate` - Convert bars or gems to cash\n"
                f"`{p}ctgive` - Transfer cash or bars to another player\n"
                f"`{p}ctconvert` - Convert gems to Discord currency"
            ),
            inline=False
        )
        ...
        # Inventory & Market
        embed.add_field(
            name="Inventory and Market",
            value=(
                f"`{p}ctinv` - View inventory, wear/remove gear\n"
                f"`{p}ctbm` - Browse and buy from the Black Market"
            ),
            inline=False
        )
        ...
        embed.add_field(
            name="Gangs",
            value=f"`{p}ctgang` - Create, join, or manage gangs",
            inline=False
        )
...
#Cttarget command, displays most recent targets of Mug
    ...
    async def cttarget(self, ctx: commands.Context):
        ...
        if recent_targets:
            target_list = "\n".join(f"║{t.ljust(inner_width)}║" for t in recent_targets)
        else:
            target_list = "║" + "  No recent targets".ljust(inner_width) + "║"
        
        lines = [
            "```",
            "╔" + border + "╗",
            "║" + "RECENT TARGETS".center(inner_width) + "║",
            "║" + f"~ {member.display_name} ~".center(inner_width) + "║",
            "╠" + border + "╣",
            target_list,
            "╠" + border + "╣",
            "║" + "Attack others NOT on this list.".center(inner_width) + "║",
            "╚" + border + "╝",
            "```"
        ]
        await ctx.send("\n".join(lines))
    ...
    # Check balance and stats specifically attributed to the Mug command.
    @commands.command(aliases=["ctstats"])
    async def ctstat(self, ctx: commands.Context, member: discord.Member = None):
        ...
        total_wealth = cash + (bars * guildsettings.bar_value) + (gems * guildsettings.gem_value)
        ...
        lines = [
            ...
            "║" + f"   Gold Bars : {bars:>3}  →   {f'${guildsettings.bar_value*bars:,}':>14}".ljust(inner_width) + "║",
            "║" + f"   Gems      : {gems:>3}  →   {f'${guildsettings.gem_value*gems:,}':>14}".ljust(inner_width) + "║",
    ...
    async def ctwealth(self, ctx: commands.Context, member: discord.Member = None):
        ...
        cash = user.balance
        gold = user.gold_bars
        gold_value = guildsettings.bar_value
        gems = user.gems_owned
        gem_value = guildsettings.gem_value
...
########## Economic/Asset Commands ##########
    ...
    async def ctconvert(self, ctx: commands.Context, amount: str = None):
        ...
        if not guildsettings.is_conversion_enabled:
        ...
        currency_to_deposit = (amount * guildsettings.gem_conversion_value)
        user.gems_owned -= amount
